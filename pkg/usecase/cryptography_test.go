...
func TestECInRangeUseCase(t *testing.T) {
	err := zlog.NewSugaredDevLogger()
	if err != nil {
		t.Fatalf("an error '%s' was not expected when opening a sugared logger", err)
	}
	...
	ecUc := NewECDetection(ctx, s, conn, myConfig)

	requestDto, err := dtos.ParseCryptoInput(s, []byte(cryptoRequest))
	if err != nil {
		t.Fatalf("an error '%s' was not expected when parsing input json", err)
	}
	usages, notFound, err := ecUc.GetDetectionsInRange(requestDto)
	fmt.Printf("%d, %v\n%+v\n", notFound, err, usages)
	if err != nil {
		t.Fatalf("the error '%v' was not expected when getting cryptography", err)
	}
	if len(usages.Hints) == 0 {
		t.Fatalf("Expected to receive  1 purl")
	}
	if len(usages.Hints[0].Versions) == 0 || len(usages.Hints[0].Versions) != 3 {
		t.Fatalf("Expected to receive  3 versions")
	}
	/*
	   	if len(algorithms.Cryptography[0].Algorithms) == 0 || notFound > 0 {
